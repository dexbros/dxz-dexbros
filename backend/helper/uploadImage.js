/** @format */

const { Storage } = require("@google-cloud/storage");
const path = require("path");

const uuid = require("uuid");
const uuidv1 = uuid.v1;

const storage = new Storage({
  projectId: "quiztasy",
  credentials: {
    client_email: "dexbros-upload@quiztasy.iam.gserviceaccount.com",
    private_key:
      "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
  },
});

const sharp = require("sharp");
// const multer = Multer({
//   storage: Multer.memoryStorage(),
//   limits: {
//     fileSize: 75 * 1024 * 1024,
//   },
// });
const bucket = storage.bucket("dexbros_files");

class UploadImage {
  constructor() {}

  // *** Upload image in google cloud
  async uploadImage(data) {
    return new Promise(async (resolve, reject) => {
      const newImageName = uuidv1() + "-" + data.originalname;
      const blob = bucket.file(newImageName);
      const blobStream = blob.createWriteStream();
      blobStream.on("error", (err) => {
        console.log(err);
        reject({ msg: err.message });
      });
      blobStream.on("finish", async () => {
        var publicURL = `https://storage.googleapis.com/${process.env.GCS_BUCKET}/${blob.name}`;

        const arr = publicURL.split("/");
        const temp = arr[arr.length - 2].split("_");
        const str = `_${temp[temp.length - 1]}/${arr[arr.length - 1]}`;
        resolve(str);
      });
      blobStream.end(data.buffer);
    });
  }

  // *** Create Folder in Google cloud
  /**
   * @GCP folder structure
   */
  // async createFolder(name) {
  //   console.log("CREATE FOLDER >>>");
  //   const path = `social/users/${name}/`;
  //   const [files] = await bucket.getFiles({ prefix: path });

  //   if (files.length > 0) {
  //     console.log("Folder already exists");
  //     return;
  //   } else {
  //     const blob = bucket.file(path);
  //     await blob.save("", {
  //       metadata: {
  //         contentType: "application/x-www-form-urlencoded;charset=UTF-8",
  //       },
  //     });
  //     console.log("File created");
  //     return;
  //   }
  // }

  /*
  async uploadImage(file) {
    console.log(file);
    const folderName = `social/acc/profile`;
    const imageName = uuidv1() + "-" + file.originalname;
    //     const blob = bucket.file(newImageName);
    //     const blobStream = blob.createWriteStream();
    const options = {
      destination: `${folderName}/${imageName}`,
    };

    bucket.upload(imageName, options, function (err, file) {
      if (err) {
        console.error(err);
        return;
      }
      var publicURL = `https://storage.googleapis.com/${process.env.GCS_BUCKET}/${blob.name}`;
      console.log(publicURL);

      // console.log(`Image ${file.name} uploaded to ${bucketName}/${folderName}`);
    });
  }
  */

  async createFolder(name, type) {
    console.log("CREATE FOLDER >>>");
    const path = `social/${name}/${type}`;
    const [files] = await bucket.getFiles({ prefix: path });

    if (files.length > 0) {
      console.log("Folder already exists");
      return;
    } else {
      const blob = bucket.file(path);
      await blob.save("", {
        metadata: {
          contentType: "application/x-www-form-urlencoded;charset=UTF-8",
        },
      });
      console.log("File created");
      return;
    }
  }
}

module.exports = new UploadImage();
